...
function initAddClasses() {
	jQuery('a.btn-select').clickClass({
		classAdd: 'active',
		addToParent: true
	});
}

function initCheckFormFill(){
	var forms = jQuery('.shipping-form, .billing-form, .logininfo-form');
	...
	function checkForm(form) {
		...
		else form.addClass('filled-success');
	}
}
...
function initTariffOrdering(){
	...
	if(columns.length) {
		columns.each(function(){
			...
			var btnSelect = holder.find('.btn-select');
			var lightbox = jQuery(btn.attr('href'));
			var price = holder.find('.monthly-rate .num').text();
			var tariffName = holder.find('h2.name').text();
			var quantity = holder.find('.photos-num em.num').text();
			var lightboxForms = lightbox.find('form');


			lightboxForms.each(function(){
				this.reset();
			});
			
			if(!lightbox.length) return;
			btn.on('click', function(e){
				$(this).parents('.column').siblings('.column').find('.btns').removeClass('active');
				if(!holder.hasClass('active')) {
					var dataHolder = lightbox.find('.you-selected');
					dataHolder.find('.name-text').text(tariffName);
					dataHolder.find('.price-text').text(price);
					dataHolder.find('.quantity-text').text(quantity);
			...
			btnSelect.on('click', function(){btn.trigger('click')});
		});
		
		var shippingForm = jQuery('.shipping-form').attr('novalidate', 'novalidate');
		var shippingSubmit = shippingForm.find('a.continue.lightbox');
		shippingSubmit.on('click', function(e){
			if(validate(shippingForm.find('input, textarea, select'))) {
				jQuery.fancybox({
					padding: 10,
					margin: 0,
		...
		var billingForm = jQuery('.billing-form').attr('novalidate', 'novalidate');
		billingForm.submit(function(e){
			if(!validate(billingForm.find('input:visible, textarea:visible, select:visible'))) {
				e.preventDefault();
			}
		});
		
		//billing slide open-close
		var checkOpener = billingForm.find('input.slide-opener');
		var slide = jQuery(checkOpener.data('slide')).css({height: 0});
		
		function toggleSlide(){
			if(checkOpener.is(':checked')) slide.stop().animate({height: 0});
			else {
				slide.stop().animate({height: slide.children().outerHeight()});
			}
		}
		toggleSlide();
		checkOpener[0].onchange = toggleSlide;
	}
...
function validate(inputs){
	...
	function checkField(i, obj) {
		...
		// correct email fields with confirmation
		if(currentObject.hasClass('required-verify-email')) {
			setState(currentParent, currentObject, !regEmail.test(currentObject.val()) || (currentObject.val() !== jQuery(currentObject.data('verify')).val()));
		}
...
function initLightbox() {
	jQuery('a.lightbox:not(.continue), a[rel*="lightbox"]:not(.continue)').each(function(){
